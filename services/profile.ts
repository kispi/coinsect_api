...
const nicknameRecommendations = [
  ...
  '코인충추종자', '세계관이박호두', '호반꿀', '셀반꿀', '나이따', '이렇게또성장', '자본주의폰지사기', '양적완화좀해줘', '금리좀내려라',
  ...
  '잠시후청산', '살때까지올린다', '팔때까지내린다', '돈은또벌면됨', '올려오늘왜이래', '관망중', '무포가상팔자', '너한테비싸게팜', '방구석워뇨띠', '파동충',
  '경기침체', '웃음기사라지제', '신이원망스럽제', '가족생각나제', '바닥이안보이제', '왜마진했나싶제', '슬슬쫄리제', '슬슬괜히탔다싶제', '타짜교수', '도박충', '금융인',
  '코인충중독자', '포지션잡지말걸', '롱물려있음', '숏물려있음', '영원히구조안옴', '평단개구데기', '와이코프', '매매못하는사람', '롱이라했제', '숏이라했제',
  '잡자마자반대로감', '개인회생예정자', '파산직전', '구조조정', '짭따꿀', '익절을모르는', '손절을모르는', '영원히관망만함', '머리에서롱', '각질에서숏', '아직발톱안나옴',
  '내가뭐랬어', '리플평단4500원', '도지평단880원', '퀀텀오르면하락장', '설거지뽀드득', '설거지라도해줘', '뽀드득소리나게', '이거내건데',
]

const nicknameService = {
  generate: (): string => {
    const randIdx = Math.floor(Math.random() * nicknameRecommendations.length)
    const randNo = Math.floor(Math.random() * 99 + 1) // 100이 나오는 경우 닉네임의 길이가 10자를 초과함.
